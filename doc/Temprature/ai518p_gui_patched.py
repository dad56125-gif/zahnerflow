    ...
    def send_command(self, command):
        ...
        with self.lock:  # 串口互斥，避免多线程同时读写
            try:
                ...
                # 仅写命令（第3字节0x43）打印详细日志
                is_write = len(command) >= 3 and command[2] == 0x43
                if is_write:
                ...
                while time.time() - start_time < max_wait and len(response) < target_len:
                    ...
                    if waiting > 0:
                        ...
                        if chunk:
                            ...
                            if is_write:
                                print(f"写入响应进度: 当前{len(response)}字节")
                    else:
                        time.sleep(0.01)  # 轻量轮询

                if len(response) >= target_len:
                    resp = bytes(response[:target_len])
                    if is_write:
                        print(f"写入响应接收: {[hex(b) for b in resp]}")
                    return resp
                else:
                    if is_write:
                        print(f"写入响应超时: 仅{len(response)}字节可用")
                        if len(response) > 0:
                            print(f"写入部分响应: {[hex(b) for b in response]}")
                    return None
