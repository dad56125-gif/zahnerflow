...
本规范保留现有主工作台布局、组件尺寸和排版。暗色延续玻璃界面；亮色采用专业冷灰白表面，以轻微明度差、细分隔线和轻阴影区分工具栏、侧栏、画布与悬浮卡片。
...
| 面板 | 暗色使用玻璃容器；亮色使用 `--surface-*` 定义工具栏、侧栏、画布、卡片与输入表面，不叠加装饰性彩色渐变 | `_placeholders.scss`、`_light-theme.scss` |
