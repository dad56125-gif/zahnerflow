...
| 面板 | 暗色使用玻璃容器；亮色使用 `--surface-*` 定义工具栏、侧栏、画布、卡片与输入表面，仅画布和工具条允许极弱冷调明暗过渡，不使用蓝紫极光 | `_placeholders.scss`、`_light-theme.scss` |
...
亮色层级修订（已完成）：恢复节点库柔和边界与轻阴影；应用底色、工具条、侧栏、画布、功能卡片分级，画布允许极弱冷调明暗过渡；属性分组与激活 Tab 加强结构，保持布局尺寸及业务逻辑。

验证：完整构建、lint、版本检查通过；两档窗口共 18 项几何比对一致；节点悬停、键盘焦点、Enter 创建、禁用点击不创建及亮暗刷新恢复通过，页面异常为空。截图与临时脚本位于 .codex-run/layer-*，未连接真机。
