...
export const LSVMeasurementNode: React.FC<NodeComponentProps> = ({ node, onUpdate }) => {
  const [parameters, setParameters] = useState(node.parameters || {
    measurement_type: MeasurementType.LSV,
    output_path: '/tmp/lsv_data',
    naming_mode: 'COUNTER',
    counter: 1,
    filename: 'lsv_data',
  ...
  return (
    <div className="node-content">
      <div className="node-header">
        ...
        <span className="node-title">线性扫描伏安法</span>
      </div>
      ...
      <div className="node-parameters">
        ...
        <ParameterInput
          label="电压参考"
          type="select"
          value={parameters.voltage_reference}
          onChange={(value) => handleParameterChange('voltage_reference', value)}
          options={[
            { value: 'absolute', label: '绝对值' },
            { value: 'ocv', label: '开路电压' }
          ]}
        />
