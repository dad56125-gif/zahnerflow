...
const ParticleBackground: React.FC<ParticleBackgroundProps> = ({ suspended = false }) => {
    ...
    const theme = backgroundPalette === 'aurora' ? 'dark' : 'light';
    ...
    useEffect(() => {
        ...
        const background = palette.getPropertyValue(theme === 'dark' ? '--aurora-base' : '--mesh-base').trim();
        const particleRgb = palette.getPropertyValue('--aurora-particle-rgb').trim();
    ...
    return (
        <canvas
            ...
            style={{
                opacity: theme === 'light' && (backgroundPalette === 'mint' || backgroundPalette === 'pink') ? 0.75 : 1,
                position: 'fixed',
                top: 0,
                left: 0,
                width: '100%',
