...
const NODE_PARAM_CONFIG: Record<string, ParamDisplayConfig> = {
    ...
    eis_potentiostatic: {
        ...
        key: 'eisPotential',
        unit: 'V',
        // 特殊处理：如果 enableDcBias 为 false 且 eisPotential 为 0，显示 OCV
        format: (v, params) => {
            if (params && !params.enableDcBias && (v === 0 || v === undefined)) {
                return 'OC';
            }
            return `${v || 0}`;
        },
    ...
    scheduled_start: {
        ...
        format: (_v, params) => {
            if (!params) return '';
            const hour = String(params.hour ?? 0).padStart(2, '0');
            const minute = String(params.minute ?? 0).padStart(2, '0');
            return `${params.nextDay ? '次日 ' : ''}${hour}:${minute}`;
        }
    },
    workflow_block: {
        label: '工作流',
        ...
        unit: '',
        format: (v, params) => {
            const name = v || params?.workflowShortId || '未选择';
            const count = Number(params?.nodeCount || 0);
            return count > 0 ? `${name} / ${count}节点` : String(name);
        }
    },
    ocp_measurement: {
        label: '时长',
    ...
    current_ramp: {
        label: '电流',
        key: 'startCurrent', // 主 key 仅用于存在性检查
        unit: '',
        // 特殊格式化：显示 起始电流 → 结束电流
        format: (_v, params) => {
            if (!params) return '';

            const startI = params.startCurrent ?? 0;
            const endI = params.endCurrent ?? 0;
    ...
    return (
        <div className="eis-parameters">
            <div className="eis-current">
                {formatValue(primaryValue, config)}
            </div>
            {config.secondary && (
