...
const CROSS_ROW_TURN_MARGIN = 8;
const BASE_NODE_HEIGHT = 48;
const LOOP_LABEL_SIDE_OFFSET_RATIO = 0.34;
const LOOP_LABEL_VERTICAL_GAP_RATIO = 0.78;
...
const LoopLabel: React.FC<LoopLabelProps> = ({ loop, position, textColor, nodeHeight }) => {
  // 使用 hook 获取当前循环的执行进度
  ...
  const labelBoundaryGap = Math.round(nodeHeight * LOOP_LABEL_VERTICAL_GAP_RATIO);
  const labelText = `第${loop.level + 1}级循环 • ${loop.iterationCount}次${progressText}`;

  return (
    <text
      x={position.x}
      y={position.y - labelBoundaryGap}
      fill={textColor}
      fontSize={labelFontSize}
      textAnchor="middle"
      className="loop-boundary__label"
    >
      {labelText}
    </text>
  );
...
export const LoopBoundary: React.FC<LoopBoundaryProps> = ({
    ...
    for (let i = 0; i < completeLoopNodes.length; i++) {
      ...
      if (i < completeLoopNodes.length - 1) {
        ...
        if (currentRow !== nextRow) {
          ...
          if (rowIsLeftToRight) {
            // 偶数行从左到右，在右侧转折
            const turnX = Math.max(centerPoint.x + nodeWidth / 2, nextCenterPoint.x + nextNodeWidth / 2) + CROSS_ROW_TURN_MARGIN;
            finalPoints.push({ x: turnX, y: centerPoint.y });
            finalPoints.push({ x: turnX, y: nextCenterPoint.y });
          } else {
            // 奇数行从右到左，在左侧转折
            const turnX = Math.min(centerPoint.x - nodeWidth / 2, nextCenterPoint.x - nextNodeWidth / 2) - CROSS_ROW_TURN_MARGIN;
            finalPoints.push({ x: turnX, y: centerPoint.y });
            finalPoints.push({ x: turnX, y: nextCenterPoint.y });
          }
        }
  ...
  return (
    <svg
      className={`loop-boundary ${className} level-${loop.level}`}
      style={{
        position: 'absolute',
        left: 0,
        top: 0,
    ...
    >
      <path
        d={beltPath}
        className="loop-boundary__fill loop-boundary__border"
      />
      <g>
        {loopInnerNodes.length > 0 && (() => {
          const firstNode = loopInnerNodes[0];
          const firstPoint = getNodeCenterPoint(firstNode);
          const firstNodeWidth = firstNode.layoutMeta?.width ?? firstNode.style?.width ?? 140;
          const firstNodeHeight = firstNode.style?.height ?? BASE_NODE_HEIGHT;
          const rowIsLeftToRight = (firstNode.layoutMeta?.row ?? 0) % 2 === 0;
          const labelPoint = {
            x: firstPoint.x + (rowIsLeftToRight ? 1 : -1) * firstNodeWidth * LOOP_LABEL_SIDE_OFFSET_RATIO,
            y: firstPoint.y,
          };
