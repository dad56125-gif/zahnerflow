...
export const MFCDeviceCard: React.FC<MFCDeviceCardProps> = ({
  ...
  // 处理模式切换
  const handleSetMode = useCallback(async (mode: 'hold' | 'follow') => {
    ...
    try {
      await onSetMode?.(device.address, mode);
    } catch (error) {
      setError(`${mode.toUpperCase()} 模式设置失败: ${error instanceof Error ? error.message : '未知错误'}`);
    } finally {
      setIsSettingMode(false);
    }
  }, [device.mode, device.address, onSetMode]);
  ...
  return (
    <div className={`mfc-device-card glass ${loading ? 'loading' : ''} ${disabled ? 'disabled' : ''}`}>
            ...
            <div className="control-buttons">
              <button
                className={`btn btn-sm ${
                  device.mode === 'hold' ? 'btn-primary' : 'btn-secondary'
                } ${isSettingMode ? 'btn-loading' : ''}`}
                onClick={() => handleSetMode('hold')}
                disabled={disabled || isSettingMode || loading || device.status !== 'connected'}
              >
                HOLD
              </button>
              <button
                className={`btn btn-sm ${
                  device.mode === 'follow' ? 'btn-primary' : 'btn-secondary'
                } ${isSettingMode ? 'btn-loading' : ''}`}
        ...
        <div className="mfc-right-panel">
          ...
          <div className="flow-percentages">
            ...
            <div className="percentage-item">
              ...
              <span className="percentage-value">
                {device.max_flow_sccm > 0
                  ? ((device.set_flow / device.max_flow_sccm) * 100).toFixed(1)
                  : '0.0'
                }%
              </span>
            </div>
      ...
      {/* 加载遮罩 */}
      {(loading || isSettingFlow || isSettingMode) && (
