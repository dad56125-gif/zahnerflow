...
const preview = (text: string): TutorialStep[] => [
  { title: '展开实际执行计划', text, target: anchor('preview'), action: 'click', check: { selector: '.unroll-row[data-step]' } },
  note('核对执行边界', '预览中的启动、停止等系统步骤，不代表已经完成气路、加湿和 OCV 判据确认。', '.unroll-dialog__sequence'),
