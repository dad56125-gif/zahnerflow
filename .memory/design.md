...
主题规则：在相同布局和组件尺寸下提供亮色和暗色两套配色，默认亮色。亮色以弱冷调浅色表面、柔和边界和轻阴影建立层级：外层底色略深，工具条悬浮，侧栏独立，画布保留淡网格与极弱冷调明暗过渡，节点库默认是亮于面板的功能卡片；背景不绘制极光或粒子；蓝色用于选中、交互和设备连接，节点类型与特殊状态保留语义色。`appStore.theme` 是唯一主题状态，沿用 `app-storage` 持久化；`main.tsx` 在首次 React 渲染前应用主题，顶栏按钮切换 `data-theme`。背景画布订阅同一状态，仅暗色启动动画，图表沿用现有主题订阅刷新；核心中性色通道和语义色均由 `_tokens.scss` 提供，阴影、遮罩和测量曲线不整体反色。
