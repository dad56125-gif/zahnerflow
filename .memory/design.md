...
主题规则：在相同布局和组件尺寸下提供亮色和暗色两套配色，默认亮色。亮色外层使用浅色渐变与分层细丝粒子波浪，工具条和侧栏使用浅色半透明磨砂玻璃、柔和高光边缘与轻阴影，画布使用半透明浅色底透出波面纹理并保留淡网格；节点库以更亮的卡片表面、柔和边框和轻阴影独立于面板，颜色集中于节点类型、设备图标和控件状态；亮色波面由细流线、竖向短丝和疏密颗粒组成，固定采样避免闪烁，整体缓慢起伏；蓝色用于选中、交互和设备连接，节点类型与特殊状态保留语义色。`appStore.theme` 是唯一主题状态，沿用 `app-storage` 持久化；`main.tsx` 在首次 React 渲染前应用主题，顶栏按钮切换 `data-theme`。背景画布订阅同一状态，亮色绘制粒子波浪，暗色保留原极光星座动画；后台、浮层暂停和系统减少动态效果设置均停止动画，图表沿用现有主题订阅刷新；核心中性色通道和语义色均由 `_tokens.scss` 提供，阴影、遮罩和测量曲线不整体反色。
