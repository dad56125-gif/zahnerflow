...
主题规则：在相同布局和组件尺寸下提供亮色和暗色两套配色，默认亮色。亮色外层使用上粉下蓝的双向浪涌背景，工具条和侧栏使用浅色半透明磨砂玻璃、柔和高光边缘与轻阴影，画布使用半透明浅色底透出波面纹理并保留淡网格；节点库以更亮的卡片表面、柔和边框和轻阴影独立于面板，颜色集中于节点类型、设备图标和控件状态；亮色背景按上 35%、下 65% 布局，35% 高度附近以增强起伏的波形和扩展的粉紫、蓝紫混色带形成柔和浪涌交界，背景画布不透明度为 0.5，背景独立使用 brightness(1.25) saturate(0.7) 提高 25% 明度并降低 30% 饱和度；在此基础上蓝色色场保持原始令牌明度（撤回额外 30% 提亮），并在当前基础上额外降低 30% 饱和度、粉色色场先降低明度 10%、再降低 20%，交界令牌在粉色边缘与蓝色表面间连续插值，避免高亮白带；上部固定 42 片向上花瓣，下部固定 110 粒白色沙砾，取消密集点线，使用低分辨率色场平滑放大和 24 帧动画，固定采样避免闪烁；玻璃面板和画布以浅色遮罩保持文字清晰；蓝色用于选中、交互和设备连接，节点类型与特殊状态保留语义色。`appStore.theme` 是唯一主题状态，沿用 `app-storage` 持久化；`main.tsx` 在首次 React 渲染前应用主题，顶栏按钮切换 `data-theme`。背景画布订阅同一状态，亮色绘制粒子波浪，暗色保留原极光星座动画；后台、浮层暂停和系统减少动态效果设置均停止动画，图表沿用现有主题订阅刷新；核心中性色通道和语义色均由 `_tokens.scss` 提供，阴影、遮罩和测量曲线不整体反色。
