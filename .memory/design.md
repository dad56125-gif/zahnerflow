...
展开浏览规则：`UnrollViewModal` 通过 `runtimeClient` 读取 `/unroll-preview`，`unrollViewModel` 只把后端原序列适配为 Finder 式分栏树，不重新展开、排序或编号。根列显示当前执行结构；选择循环或高级节点后在右侧追加子列，选择最终步骤后最右侧追加详情与启动操作。循环和高级节点分组必须显示其成员覆盖的真实步骤号范围，不能因收起而造成序号视觉断档。工作流块在后端计划中已被展开，Finder 导航不得再把它收起成块或增加一列；其真实子步骤直接进入所在循环或根层级，`blockPath` 只保留给详情来源。循环组身份由完整迭代路径确定，不把工作流块路径拼入已激活的外层循环，连续 occurrence 继续隔离重复引用。改变左侧选择会替换其后的路径；窄屏保留横向分栏并自动滚到新列或详情，不回退为另一套上下布局。完整计划中的自动 `startup` / `shutdown` 保留为可以检查但不能手动启动的系统边界，并在展示上归入最近的真实步骤结构；普通步骤继续使用真实 `unrolledIndex` 作为选择和启动身份。界面不提供搜索，只保留按真实步骤编号定位并恢复完整分栏路径。外层、标题、主体、关闭按钮和背景使用共享 modal 样式，Finder 只维护内部导航布局。`useUnrollPreview` 丢弃过期响应。启动回调显式返回结果，modal 只有在后端启动成功后关闭；缺少运行信息或启动失败时保留所选起点供再次确认。
...
颜色规则：状态颜色及透明派生以 _tokens.scss 中的主题 RGB 变量为源，组件和按钮变体不得另写相同语义颜色。图表系列使用独立 chart 令牌，避免将测量系列色误当成功状态色；导出纸张、数据系列调色板与透明度蒙版不随界面状态色替换。


主题规则：在相同布局和组件尺寸下提供亮色和暗色两套配色，默认亮色。亮色外层使用多色团动态网格渐变背景，工具条和侧栏使用浅色半透明磨砂玻璃、柔和高光边缘与轻阴影，画布使用半透明浅色底透出波面纹理并保留淡网格；节点库以更亮的卡片表面、柔和边框和轻阴影独立于面板，颜色集中于节点类型、设备图标和控件状态；亮色背景使用雾蓝、薄荷、浅粉、奶杏、灰蓝和淡紫六个径向色团叠加，中心沿连续周期轨迹缓慢漂移；移除旧上下浪涌、粒子和全局调色滤镜，使用低分辨率色场平滑放大及 24 帧动画；玻璃面板和画布以浅色遮罩保持文字清晰；蓝色用于选中、交互和设备连接，节点类型与特殊状态保留语义色。背景配色由 `appStore.backgroundPalette` 管理，个人偏好高光渐变胶囊提供雾蓝、薄荷、樱粉、奶杏混色和经典极光，通过 `app-storage` 保存并在刷新后恢复；所有主题背景订阅该选择，薄荷与樱粉背景不透明度为 75%，雾蓝与奶杏混色保持 100%。`appStore.theme` 是唯一主题状态，沿用 `app-storage` 持久化；`main.tsx` 在首次 React 渲染前应用主题，顶栏不再提供亮暗切换按钮，选择经典极光同步切换深色主题，选择其余背景同步切换浅色主题，背景与主题一起持久化。背景画布订阅同一状态，亮色绘制动态网格渐变，所有主题均使用所选背景，经典极光选项恢复原深色极光星座动画；后台、浮层暂停和系统减少动态效果设置均停止动画，图表沿用现有主题订阅刷新；核心中性色通道和语义色均由 `_tokens.scss` 提供，阴影、遮罩和测量曲线不整体反色。

当前规则：`styles/_tokens.scss` 是颜色、字体、间距、圆角、动效和层级的核心令牌入口；`_base.scss` 只保留基础规则和响应式布局覆盖。`main.scss` 通过 Sass `meta.load-css` 在既有基础、布局、组件三层中加载模块；每个模块显式引入所用占位符。节点库、画布和展开步骤共用 `_node-icons.scss`；无变体按钮也必须具备当前主题的基础外观；亮色的 `_light-theme.scss` 只适配表面和状态反馈，不改变几何。滚动行不叠加独立模糊层。
