...
主题规则：在相同布局和组件尺寸下提供亮色和暗色两套配色，默认亮色。亮色外层使用多色团动态网格渐变背景，工具条和侧栏使用浅色半透明磨砂玻璃、柔和高光边缘与轻阴影，画布使用半透明浅色底透出波面纹理并保留淡网格；节点库以更亮的卡片表面、柔和边框和轻阴影独立于面板，颜色集中于节点类型、设备图标和控件状态；亮色背景使用雾蓝、薄荷、浅粉、奶杏、灰蓝和淡紫六个径向色团叠加，中心沿连续周期轨迹缓慢漂移；移除旧上下浪涌、粒子和全局调色滤镜，使用低分辨率色场平滑放大及 24 帧动画；玻璃面板和画布以浅色遮罩保持文字清晰；蓝色用于选中、交互和设备连接，节点类型与特殊状态保留语义色。背景配色由 `appStore.backgroundPalette` 管理，个人偏好高光渐变胶囊提供雾蓝、薄荷、樱粉、奶杏混色，通过 `app-storage` 保存并在刷新后恢复；所有主题背景订阅该选择，薄荷与樱粉背景不透明度为 75%，雾蓝与奶杏混色保持 100%。`appStore.theme` 是唯一主题状态，沿用 `app-storage` 持久化；`main.tsx` 在首次 React 渲染前应用主题，顶栏不再提供亮暗切换按钮，保留现有持久化主题状态。背景画布订阅同一状态，亮色绘制动态网格渐变，暗色同样使用所选动态网格渐变；后台、浮层暂停和系统减少动态效果设置均停止动画，图表沿用现有主题订阅刷新；核心中性色通道和语义色均由 `_tokens.scss` 提供，阴影、遮罩和测量曲线不整体反色。
