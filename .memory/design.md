...
当前规则：顶栏新建用户按钮右侧提供新手教程入口。目录沿用实验记录的共享 SplitPaneModal，左侧分类目录，右侧课程概述与学习阶段时间线。每门课程必填 outline，引用 tutorialOutlines.ts 中的总结性阶段说明；相邻操作按学习目的合并，左侧不显示步骤或阶段数。教程弹窗限定最大 960×660px，按视口收缩，桌面目录约占 26%。实际演示仍读取 steps 的完整动作与验收条件，不能把说明阶段当成执行脚本。开始演示时收起目录，在当前已挂载的 App 上显示教学控制、光标与聚光标注。主页面、Canvas、Toolbar、RightPanel、用户配置、执行预览、图表和实验记录沿用真实组件及处理链，不创建 iframe、第二个 React 根或业务控件副本。
...
当前规则：doc/README.md 是当前独立教学分支的文档导航；保留基线文档布局，不引入其他分支的文档搬迁。架构事实由本文件维护，演进原因与证据由 changelog.md 维护，应用发布变化由根 CHANGELOG.md 维护。doc/architecture/source-of-truth.md 当前限定教学功能的源头与派生链，明确源码、模拟器采集数据和开发验收录屏的不同职责，不声称覆盖全项目来源审计。
